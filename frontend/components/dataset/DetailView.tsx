...
function decideAbility(request: DatasetRequest, roles: string[], userId: string) {
  ...
  switch (request.currentTaskType) {
    /**
     * ด่านเดียวของผู้ประสานงานของ BDI — ส่งต่อแล้วไปหาผู้มีอำนาจของหน่วยงานเสมอ
     *
     ...
     */
    /**
     * ด่านเดียวที่ฝั่ง BDI มีในเส้นทางนี้ และเป็นของผู้ประสานงานของ BDI คนเดียว
     *
     ...
     * เป็นของผู้ประสานงานของ BDI ตลอดเวลา ไม่ต้องรอความเห็นและไม่ต้องถอนการมอบหมายก่อน
    ...
    case "BDI_OFFICER_REVIEW":
      ...
      if (isSpecialist) {
        return {
          ...
          advanceLabel: null,
          /**
           * บอกสิ่งที่ต้องทำ ไม่ใช่เล่าขั้นตอนการทำงานให้ฟัง (BDI ขอเมื่อ 2026-09-11)
           *
           * ของเดิมต่อท้ายด้วย "การตัดสินผ่านหรือส่งกลับเป็นหน้าที่ของผู้ประสานงานของ BDI"
           * ซึ่งยังจริง — ด่านนี้เป็นของผู้ประสานงานตลอดเวลา (2026-08-30) — แต่เป็นกติกา
           * ภายในที่ผู้เชี่ยวชาญไม่ต้องอ่านซ้ำทุกครั้งที่เปิดคำขอ และการที่หน้าจอไม่มีปุ่ม
           * ผ่าน/ส่งกลับให้เขากดอยู่แล้วก็บอกเรื่องเดียวกันโดยไม่ต้องเขียน
           */
          hint: "กรุณาบันทึกความเห็นเพื่อให้ผู้ประสานงานของ BDI ใช้ประกอบการตัดสินใจ",
          canRevise: false,
          canAssign: false,
          canComment: true,
          canReject: false,
...
export function DatasetDetailView({ id, backHref }: { id: string; backHref?: string }) {
   ...
   *
   * **ผู้อนุมัติ BDI ไม่เห็นกล่องนี้** (BDI ขอเมื่อ 2026-09-10) ความเห็นของผู้เชี่ยวชาญ
   * เป็นสิ่งที่ *ผู้ประสานงานของ BDI* ขอไว้ประกอบการตัดสินใจของตัวเองที่ด่านที่ 2 การยกมันขึ้นมา
   * ไว้บนสุดของหน้าคือทางลัดสำหรับเจ้าหน้าที่คนนั้น พอถึงด่านที่ 4 เจ้าหน้าที่ตัดสินไปแล้ว
   * ความเห็นนั้นจึงไม่ใช่สิ่งที่ผู้อนุมัติต้องชั่งน้ำหนักใหม่ — และการวางไว้บนสุดทำให้มัน
   * อ่านเหมือนข้อเสนอที่รอการตัดสิน
   *
   * นี่เป็นการจัดหน้าจอ **ไม่ใช่กฎเรื่องสิทธิ์** — แถวเดิมยังอยู่ในไทม์ไลน์ตามปกติ
          ...
          action === "approve"
            ...
            : action === "reject"
              ...
              : action === "comment"
                ...
                : "ส่งกลับให้แก้ไขแล้ว",
        /**
         * ความเห็นไม่ได้ทำให้คำขอเดินไปไหน ผู้รับจึงไม่ใช่ "ผู้เกี่ยวข้อง" ทั้งวง แต่เป็น
         * ผู้ประสานงานของ BDI ที่กำลังตัดสินใจอยู่ — ก่อนหน้านี้ทั้งสี่ปุ่มใช้ประโยคเดียวกัน
         * ทั้งที่เส้นทาง comment ไม่ได้แจ้งใครเลยสักคน
         */
        detail:
          action === "comment"
            ? "ระบบแจ้งผู้ประสานงานของ BDI ทางอีเมลและในระบบแล้ว"
            : "ระบบแจ้งผู้เกี่ยวข้องทางอีเมลและในระบบแล้ว",
      });
      closeModal();
      await load();
    } catch (err) {
    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      ...
      */}
        ...
        {request.assignedSpecialist ? (
          <Card>
            <CardHeader
              ...
              description="ผู้ประสานงานของ BDI ขอความเห็นไว้ประกอบการตัดสินใจ — ไม่ใช่ขั้นตอนที่คำขอต้องรอ"
            />
            <Rows
              rows={[
                [
      ...
      {/*
        ...
        ถ้อยคำมาจาก BDI ทั้งก้อน (2026-09-11) — ประโยคเดียว ไม่มีบรรทัดคำอธิบายใต้หัวข้อ
        ของเดิมอธิบายกลไกไว้ด้วยว่าฉบับใหม่จะกลายเป็น "ความเห็นล่าสุด" ที่ผู้ประสานงานเห็น
        และของเดิมยังอ่านย้อนหลังได้ ซึ่งยังจริงทั้งคู่ แต่ไม่ใช่สิ่งที่คนกำลังกดยืนยันต้องอ่าน
        ตรงนั้น
      */}
      <Modal
      ...
      >
        <p className="text-[15px] leading-relaxed text-ink-muted">
          ความเห็นเดิมที่ถูกบันทึกไว้จะถูกแทนที่ด้วยฉบับนี้
        </p>
        <div className="mt-6 flex justify-end gap-3">
