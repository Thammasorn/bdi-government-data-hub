...
export function Timeline({
  ...
  for (const e of events) {
    /**
     * ด่านผู้ประสานงานของ BDI ถูกเปิดด้วยการกดนำส่ง — วาดการนำส่งรอบนั้นก่อนผลการตรวจของมัน
     *
  ...
  /**
   * เรียงตามเวลาที่เกิดจริง ไม่ใช่ตามลำดับที่แถวถูกสร้าง
   *
   * `taskHistory()` เรียงด้วย `sequence_number` ซึ่งเป็นลำดับที่ **แถวเกิดในตาราง** ไม่ใช่ลำดับ
   * ที่เหตุการณ์เกิดกับผู้ใช้ ปกติสองอย่างนี้ตรงกันเพราะด่านถัดไปเปิดหลังด่านก่อนหน้าปิด แต่
   * ความเห็นของผู้เชี่ยวชาญไม่ใช่ด่าน — `recordAdvisoryNote()` เขียนแถวที่ปิดตั้งแต่เกิด ระหว่าง
   * ที่ด่านเจ้าหน้าที่ยังเปิดอยู่ แถวจึงมี sequence สูงกว่าทั้งที่เวลาเก่ากว่า บนคำขอจริงอ่านได้ว่า
   * 13:59 → 14:29 → 15:24 → 14:38 → 15:03 (การ์ด "แก้ประวัติการดำเนินงานของชุดข้อมูล")
   *
   * ต้องเรียงที่นี่ ไม่ใช่ที่ backend: หนึ่งแถวของ `review_task` กลายเป็นได้สองบรรทัดที่ถือเวลา
   * คนละคอลัมน์ (`assignedAt` ของการนำส่ง กับ `completedAt` ของผล) และบรรทัด "สร้างคำขอ" ไม่ได้
   * มาจากตารางนั้นเลย ที่นี่คือที่เดียวที่รู้เวลาที่จะ **แสดงจริง** ของทุกบรรทัด
   *
   * `Array.prototype.sort` เสถียร บรรทัดที่เวลาเท่ากันจึงคงลำดับที่ push ไว้ — ซึ่งเป็นลำดับที่
   * ถูกอยู่แล้วสำหรับคู่ "นำส่งคำขอ" กับผลของด่านเดียวกันที่ปิดในวินาทีเดียวกัน
   */
