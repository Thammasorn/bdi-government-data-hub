...
function AccountCreationStep({ token, invitation }: { token: string; invitation: InvitationInfo }) {
   ...
   * BDI กรอกไว้ตอนเชิญเมื่อ ThaID ไม่ได้ส่ง claim นั้นมา ช่องที่มีค่าแล้วจึงถูกล็อก
   * (การ์ด "แก้ form user registration") **คำนำหน้าอยู่ในกลุ่มนี้ตั้งแต่ 2026-09-07**
   * เพราะ scope มี `title` แล้ว (การ์ด "ใช้ title จาก thaid") ส่วนเบอร์โทร ThaID ไม่มีให้
   * ยังแก้ได้ตามเดิม
   *
    ...
    >
      <form onSubmit={onSubmit} className="flex flex-col gap-5" noValidate>
        ...
        {/*
          ...
          (เกิดได้เมื่อกรมการปกครองไม่ส่ง claim ของช่องนั้นกลับมา)
        */}
        {locked.prefix || locked.firstName || locked.lastName ? (
          <p className="-mt-2 text-[13px] leading-relaxed text-ink-muted">
            ...
            หากไม่ตรงกับบัตรประชาชนของคุณ กรุณาติดต่อเจ้าหน้าที่ BDI ที่เชิญคุณเข้าระบบ
          </p>
        ) : null}
