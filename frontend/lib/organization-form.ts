...
export function validateOrganizationField(
  ...
  switch (field) {
    ...
    case "signatoryEmail": {
      ...
      if (!isValidEmail(trimmed)) return "รูปแบบอีเมลไม่ถูกต้อง";
      /**
       * ผู้ดำเนินการกับผู้มีอำนาจกระทำการแทนต้องเป็นคนละคน — กฎเดียวกับ
       * `submitSchema.superRefine()` ฝั่ง API (*หนึ่งผู้ใช้ = หนึ่งบทบาท* 2026-09-03)
       *
       * ช่องอีเมลผู้ดำเนินการเป็นช่องอ่านอย่างเดียว ระบบเติมจากบัญชีที่ล็อกอินอยู่ กฎนี้
       * จึงเท่ากับ "อย่ากรอกอีเมลตัวเองในช่องผู้มีอำนาจฯ" — บอกตั้งแต่ตอนกรอกดีกว่า
       * ปล่อยไปล้มตอนกดนำส่ง เพราะกว่าจะถึงตรงนั้นต้องสร้าง PDF ไปแล้วหนึ่งรอบ
       */
      if (trimmed.toLowerCase() === values.contactEmail.trim().toLowerCase())
        return (
 ...
 *
 * อีเมลหน่วยงานและอีเมลผู้มีอำนาจกระทำการแทนต่างขึ้นกับช่องอื่น การตรวจทีละช่องตอนที่ค่านั้นเปลี่ยน
