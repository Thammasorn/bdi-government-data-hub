...
export function taskEventLabel(
   ...
   * ผู้ประสานงานของ BDI ยกเลิกผลการตรวจสอบของตัวเอง ซึ่งปิดด่านของ **คนอื่น** แทนเขา
   * แถวนี้แสดงชื่อผู้กดจาก `completed_by` อยู่แล้ว ถ้าประโยคยังเดาจาก task_type
   * มันจะเรียกผู้ประสานงานของ BDI ว่าผู้มีอำนาจอนุมัติของหน่วยงานในบรรทัดเดียวกัน
   */
  if (recalled) {
  ...
  /**
   * ด่านผู้เชี่ยวชาญปิดด้วย CONFIRMED เหมือนกับตอนที่ผู้ประสานงานของ BDI กด `confirm`
   * แต่เป็นคนละเหตุการณ์กัน — การมอบหมายผู้เชี่ยวชาญไม่ใช่ด่าน (ดู CLAUDE.md) เขา
   * **ให้ความเห็น** ไม่ได้ตัดสินผลการตรวจสอบ คำว่า "ยืนยันผลการตรวจสอบ" จึงอ่านเหมือน
   * มีคนตัดสินคำขอไปแล้วสองคน แยกเคสตรงนี้แทนที่จะแก้ตารางผลลัพธ์ทั้งใบ ซึ่งจะลากไป
   * เปลี่ยนบรรทัดของผู้ประสานงานของ BDI ที่ปิดด่านด้วย CONFIRMED เหมือนกันไปด้วย
   */
  if (taskType === "DATASET_SPECIALIST_REVIEW" && result === "CONFIRMED") {
    return `${actor}${gap}ให้ความเห็น`;
  }
