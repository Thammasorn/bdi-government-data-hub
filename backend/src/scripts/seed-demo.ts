...
async function main() {
  ...
  function datasetMetadataFixture(spec: DatasetSpec) {
    const values = normaliseMetadata({
      ...
      title: spec.title,
      name: "Sample dataset",
      dataFields: "รหัสหน่วยบริการ, ชื่อหน่วยบริการ, ประเภทที่ตั้ง, พิกัดละติจูด, พิกัดลองจิจูด, จังหวัด, อำเภอ, ตำบล",
    ...
    },
    {
      ...
      metadata: {
        ...
        allowTransformedRawDataSharingSpecifiedPlatforms:
          "ระบบบูรณาการฐานข้อมูลประชาชนและการบริการภาครัฐ (Linkage Center), ศูนย์กลางข้อมูลเปิดภาครัฐ (data.go.th)",
