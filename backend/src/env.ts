...
export const env = {
  ...
  thaid: {
     ...
     *
     * **`title` เข้ามาเมื่อ 2026-09-07** ตามการ์ด "ใช้ title จาก thaid" คำนำหน้าจึงมาจาก
     * บัตรเหมือนชื่อกับนามสกุล ไม่ใช่ dropdown ที่ผู้ใช้เลือกเองอีกต่อไป (ก่อนหน้านี้
     * รายการตามการ์ด Enhance มีหกตัว ไม่มี `title` — `docs/07` §4.1 เก็บประวัติไว้)
     *
     * ยิงจริงกับ client ของโครงการเมื่อ 2026-09-07: scope ชุดนี้ได้ 302 ไปหน้า QR
     * ส่วน `openid nonsense_scope_zzz` ยังได้ 400 `invalid_scope` — ด่านตรวจของ
     * กรมการปกครองยังทำงาน ผลข้างบนจึงไม่ใช่ระบบปล่อยผ่านทุกอย่าง
     *
     * ไม่ขอ `title_en` `middle_name` `name` `name_en` แม้ทะเบียนจะให้ได้ เพราะไม่มี
     * ที่เก็บในบัญชีและไม่มีหน้าจอไหนใช้ (`toIdentity()` ยังอ่าน `name` / `name_en` อยู่
     * ถ้าวันหนึ่งกรมการปกครองส่งมาให้เองก็ได้ค่าเพิ่มมาฟรี ไม่ต้องแก้โค้ด)
     */
    scope: optional(
      "THAID_SCOPE",
      "openid pid given_name family_name given_name_en family_name_en title",
    ),
    /**
   ...
   *
   * อีเมลมาจากการ์ด "แก้เนื้อหา invitation email" ส่วนเบอร์โทรยังไม่มีใครให้มา และทั้ง
   * repo ก็ไม่มีเบอร์ของสำนักงานเก็บไว้ที่ไหน — ค่าว่างแปลว่า "ไม่ต้องพิมพ์วรรคเบอร์โทร"
