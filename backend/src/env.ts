...
export const env = {
  ...
  thaid: {
    ...
    /**
     * เทียบเลขบัตรจาก ThaiD กับ `user_account.cid` หรือไม่
     *
     * §2.4 ของการ์ดกำหนดให้เทียบ ค่าตั้งต้นจึงเป็น true — แต่จะเทียบได้ต้องมี claim `pid`
     * ซึ่งมาเฉพาะกับ client ที่กรมการปกครองอนุมัติ scope `pid` ให้ client ของโครงการนี้
     * ยังไม่ได้รับ: ขอ scope ที่มี `pid` แล้วได้ 400 invalid_scope ตั้งแต่ขั้น authorize
     * (ทดสอบซ้ำอีกครั้ง 2026-08-16 ยังเป็นเหมือนเดิม) ส่วน `openid` เฉย ๆ ผ่าน
     *
     * ตั้ง false เพื่อให้ deployment ที่ถือ client ชุดนั้นเดิน flow ได้จนจบ โดยผูกบัญชี
     * ด้วย `sub` จาก id_token แทนเลขบัตร **นี่คือการลดระดับความปลอดภัยโดยตั้งใจ**:
     * การยืนยันจะพิสูจน์แค่ว่าผู้ใช้ยืนยันตัวตนกับ ThaiD ได้จริง ไม่ได้พิสูจน์ว่าเขาคือ
     * เจ้าของบัตรใบที่เจ้าหน้าที่บันทึกไว้ — ใครก็ตามที่ได้ลิงก์เปิดใช้งานไปและมีบัญชี
     * ThaiD ของตัวเอง จะเปิดใช้งานบัญชีนั้นได้ ยอมรับได้เฉพาะช่วงเดโม/SIT
     * ต้องกลับเป็น true ทันทีที่กรมการปกครองอนุมัติ scope `pid`
     *
     * ที่ false **ไม่ได้** ทำคือปิดการปฏิเสธเลขบัตรที่ขัดกัน — ถ้า ThaiD ส่ง `pid` มาจริง
     * และไม่ตรงกับที่บันทึกไว้ ยังยกเลิกคีย์เหมือนเดิม มีหลักฐานเมื่อไรก็ตัดสินเมื่อนั้น
     */
    requireCidMatch: optional("THAID_REQUIRE_CID_MATCH", "true") === "true",
