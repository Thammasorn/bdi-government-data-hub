 ...
 *
 * **ตั้งแต่ 2026-09-07 คำนำหน้ามาจาก claim `title` ของ ThaID** (การ์ด "ใช้ title จาก
 * thaid" — scope มี `title` แล้ว) callback เขียนทั้งสามช่องลงบัญชีทันทีที่เทียบเลขบัตร
 * ผ่าน ทั้งสามช่องจึงล็อกพร้อมกันตามปกติ และผู้มีอำนาจกระทำการแทนก็ไม่ได้รับคำนำหน้า
 * ที่เจ้าหน้าที่ของหน่วยงานกรอกแทนเขามาติดบัญชีอีกแล้ว (`ensureApproverAccount()`
 * สร้างบัญชีด้วยอีเมลกับเลขบัตรเท่านั้น)
 *
 * ค่าที่ "อยู่บนบัญชีแล้ว" จึงมาจาก ThaID เป็นหลัก เหลือทางที่สองคือชื่อที่เจ้าหน้าที่ BDI
 * กรอกไว้ตอนเชิญบัญชีภายใน (`POST /api/admin/invitations`) ซึ่ง claim จากบัตรเขียนทับ
 * ได้อยู่แล้วถ้ามาไม่ตรงกัน — ทั้งสองทางไม่ใช่สิ่งที่เจ้าของบัญชีพิมพ์เอง
 *
 * ช่องที่ยังว่างต้องปล่อยให้กรอก มิฉะนั้นเปิดใช้งานบัญชีไม่ได้เลย — เกิดกับคำนำหน้าได้
 * เมื่อกรมการปกครองไม่ส่ง claim `title` กลับมาแม้จะขอ scope ไปแล้ว
 */
...
authRouter.get("/invitation", async (req, res) => {
  ...
  res.json({
     ...
     *
     * ของผู้มีอำนาจกระทำการแทนมาจาก ThaID ล้วน ๆ — `ensureApproverAccount()` เปิดบัญชี
     * ให้ด้วยอีเมลกับเลขบัตรเท่านั้น ไม่เอาสิ่งที่เจ้าหน้าที่ของหน่วยงานกรอกแทนเขามาด้วย
     * (การ์ด "ใช้ title จาก thaid") ที่นี่จึงส่งกลับสิ่งที่ callback เขียนลงบัญชีตอนยืนยัน
     * ตัวตน ส่วนบัญชีภายในของ BDI ยังได้ชื่อที่เจ้าหน้าที่กรอกไว้ตอนเชิญเป็นค่าตั้งต้น
     *
...
authRouter.post("/thaid/callback", async (req, res) => {
   ...
   *
   * เขียนเฉพาะ claim ที่มาจริง — claim ที่ว่างเปล่าต้องไม่ไปล้างชื่อที่เจ้าหน้าที่ BDI
   * กรอกไว้ตอนเชิญทิ้ง ตั้งแต่ 2026-09-07 scope มี `title` แล้ว คำนำหน้าจึงมาทางนี้
   * เหมือนชื่อกับนามสกุล และ**ทับค่าที่มีอยู่เดิมบนบัญชี** เพราะบัตรมีน้ำหนักกว่าสิ่งที่
   * คนอื่นกรอกแทน (การ์ด "ใช้ title จาก thaid")
   */
  const fromCard = {
  ...
  res.json({
     ...
     *
     * `fullName` มาจาก claim `name` ซึ่งไม่ได้อยู่ใน scope ที่ขอ จึงเป็น null ตามปกติ
     * ปล่อยไว้เผื่อกรมการปกครองส่งมาให้เอง ส่วน `prefix` มาจาก claim `title` ที่อยู่ใน
     * scope แล้วตั้งแต่ 2026-09-07 (`docs/07` §4.1)
     */
    profile: {
