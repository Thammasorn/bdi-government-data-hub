...
const ACTIVATION_FAILURE_MESSAGES: Record<string, string> = {
  expired: "ลิงก์คำเชิญหมดอายุแล้ว กรุณาติดต่อผู้ประสานงานของ BDI เพื่อขอลิงก์ใหม่",
  used: "ลิงก์คำเชิญนี้ถูกใช้ไปแล้ว หากคุณมีบัญชีอยู่แล้วให้เข้าสู่ระบบ",
  revoked: "ลิงก์คำเชิญนี้ถูกยกเลิกแล้ว กรุณาติดต่อผู้ประสานงานของ BDI",
  ...
  if (parsed.data.purpose === "activate") {
    ...
    if (!key.userAccount.cid) {
      res.status(409).json({
        error: "cid_missing",
        message:
          "บัญชีนี้ยังไม่มีเลขประจำตัวประชาชนบันทึกไว้ จึงเทียบกับ ThaID ไม่ได้ กรุณาติดต่อผู้ประสานงานของ BDI",
      });
      return;
    }
    subjectId = key.id;
  ...
  if (key.userAccount.cid !== identity.pid) {
    ...
    res.status(403).json({
      ...
      message:
        ...
        "ลิงก์นี้ถูกยกเลิกแล้วเพื่อความปลอดภัย กรุณาติดต่อผู้ประสานงานของ BDI เพื่อขอลิงก์ใหม่",
    });
    return;
  }
