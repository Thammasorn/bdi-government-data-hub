  ...
  res.json({
    organization: {
      ...
      events: tasks.map((t) => ({
         ...
         * "ผู้มีอำนาจกระทำการแทนของหน่วยงานขอให้ปรับปรุง" ทั้งที่ชื่อผู้กระทำในบรรทัดเดียวกัน
...
async function approverConflict(
  ...
  if (heldElsewhere) {
    return {
      ...
      message:
        "อีเมลนี้ใช้เป็นผู้มีอำนาจกระทำการแทนของหน่วยงานไม่ได้ เนื่องจากมีบทบาทอื่นในระบบอยู่แล้ว " +
        "และผู้ใช้หนึ่งคนมีได้บทบาทเดียว กรุณากรอกอีเมลของผู้มีอำนาจกระทำการแทนของหน่วยงานโดยตรง",
    };
  }
...
async function recallRefusal(
  ...
  if (task.taskType !== ReviewTaskType.ORGANIZATION_APPROVAL) {
    return {
      ...
      error: "invalid_state",
      message:
        `ยกเลิกผลการตรวจสอบได้เฉพาะตอนที่คำขอรอผู้มีอำนาจกระทำการแทนของหน่วยงานลงนาม — ` +
  ...
  if (task.status !== ReviewTaskStatus.PENDING) {
    return {
      ...
      message:
        "ผู้มีอำนาจกระทำการแทนของหน่วยงานเปิดคำขอนี้แล้ว จึงยกเลิกผลการตรวจสอบแทนเขาไม่ได้ — " +
  ...
  if (request.approverEmail) {
    ...
    if (account?.status === UserAccountStatus.ACTIVE) {
      return {
        ...
        message:
          "ผู้มีอำนาจกระทำการแทนของหน่วยงานเปิดใช้งานบัญชีแล้ว อีเมลและเลขบัตรประชาชนจึงยืนยันแล้วว่าถูกต้อง " +
          "ถ้าชื่อหรือตำแหน่งผิด ให้เขากดส่งกลับแก้ไขเอง และถ้าหน่วยงานต้องการเปลี่ยนตัวผู้มีอำนาจกระทำการแทน " +
          "ให้แจ้งผู้ดูแลระบบ",
      };
    }
  }
...
async function releaseApproverSeat(
  ...
  if (account.status === UserAccountStatus.ACTIVE) {
    throw new WorkflowError(
      "approver_active",
      "บัญชีผู้มีอำนาจกระทำการแทนเปิดใช้งานแล้ว ปล่อยที่นั่งด้วยวิธีนี้ไม่ได้",
      409,
    );
  }
