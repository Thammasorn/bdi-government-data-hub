 ...
 *
 * จนถึง 2026-08-30 Journey C มีด่านผู้ประสานงานของ BDI สองด่าน (ตรวจเบื้องต้น กับ "ตรวจซ้ำ"
...
/**
 * `RETURNED` = "ด่านนี้เคยถูกทำไปแล้ว แต่ผลของรอบนั้นถูกล้างไปเพราะคำขอเดินถอยกลับ"
 *
 * ต่างจาก `UPCOMING` ที่แปลว่ายังไม่มีใครแตะเลย และต่างจาก `DONE` ที่แปลว่าผ่านไปแล้ว
 * จริง ๆ — ด่านที่อยู่ในสถานะนี้จะถูกทำใหม่ทั้งด่านเมื่อหน่วยงานนำส่งอีกครั้ง
 */
...
export interface StepPlan {
   ...
   *
   * `waitingLabel` เขียนไว้ให้อ่านเป็นประโยค ("รอผู้ประสานงานของ BDIตรวจสอบเอกสาร")
 ...
 *
 * สามด่านเท่ากับเส้นทางหน่วยงานทุกประการ — ด่านตรวจซ้ำของผู้ประสานงานของ BDI ที่เคยคั่นระหว่าง
...
export function buildJourneyProgress(params: {
  ...
  const steps: JourneyStep[] = plan.map((step) => {
    ...
    if (isCurrent) {
      ...
      state = "REJECTED";
      /**
       * คำขอเดินถอยกลับไปรอหน่วยงานแก้ไข — ด่านที่ทำไปแล้วทุกด่านถูกล้าง
       *
       * สองทางที่มาถึงตรงนี้ และทั้งคู่คือการ์ด "Bug status คำขอหลังจาก rollback หรือ
       * ส่งตรวจสอบ" (2026-09-10):
       *
       * 1. ด่านนี้เองเป็นคนส่งกลับ (`RETURNED`) — เดิมตกไปที่ `UPCOMING` แล้วขึ้นว่า
       *    "ยังไม่เริ่ม" ทั้งที่มีคนอ่านและตัดสินไปแล้ว
       * 2. ด่านอื่นส่งกลับ แต่ด่านนี้ผ่านไปแล้วในรอบเดียวกัน — เดิมค้างผล `PASSED` ไว้
       *    จึงขึ้นเขียวว่า "เสร็จสิ้น" ทั้งที่พอหน่วยงานนำส่งใหม่ ด่านนี้ต้องตรวจใหม่ทั้งด่าน
...
export interface JourneyProgressSummary {
   ...
   *
   * badge ที่ใส่ชื่อเต็ม ("รอผู้ประสานงานของ BDI ตรวจสอบเอกสาร") ล้นคอลัมน์ 12rem แล้วไป
