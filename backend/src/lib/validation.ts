...
/**
 * ตำแหน่งของผู้มีอำนาจกระทำการแทน — ต้องมีภาษาไทย ปนภาษาอังกฤษและตัวเลขได้
 *
 * เหตุผลเดียวกับ `organizationNameSchema`: ค่านี้ถูกพิมพ์ลงเอกสารข้อตกลง A0 ตรง ๆ ผ่าน
 * `org_approver.position` ซึ่งเป็นเอกสารราชการภาษาไทยที่มีการลงนาม ตำแหน่งที่กรอกมาเป็น
 * อังกฤษล้วน (เช่น "Director General") จึงเป็นข้อความผิดภาษาในเอกสารที่แก้ทีหลังไม่ได้
 * ป้ายกำกับของช่องนี้บอกไว้แล้วว่า "ตำแหน่ง (ชื่อเต็มภาษาไทย)" — กฎนี้คือการบังคับตามนั้น
 *
 * ไม่ได้ห้ามอักษรอื่น เพราะตำแหน่งราชการจริงมีตัวย่ออังกฤษปนอยู่เสมอ
 * (เช่น "ผู้อำนวยการศูนย์เทคโนโลยีสารสนเทศและการสื่อสาร (ICT)")
 *
 * ตำแหน่งของ **ผู้กรอกข้อมูล** (`contactPosition`) ไม่ใช้กฎนี้โดยตั้งใจ — ชื่อผู้กรอก
 * ไม่ได้ไปอยู่ในเอกสารที่ลงนาม และป้ายกำกับของช่องนั้นก็ไม่ได้ขอชื่อเต็มภาษาไทย
 */
export const signatoryPositionSchema = z
  .string()
  .trim()
