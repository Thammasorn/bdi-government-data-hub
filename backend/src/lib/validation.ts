...
/**
 * มีอักษรภาษาอังกฤษอยู่ในข้อความหรือไม่ — คู่แฝดของ containsThai()
 *
 * ใช้กับช่องที่ป้ายกำกับบอกว่า "ภาษาอังกฤษ" เกณฑ์เป็นแบบเดียวกัน คือ **มี**อักษร
 * อังกฤษ ไม่ใช่ **เป็น**ภาษาอังกฤษล้วน เพราะชื่อภาษาอังกฤษจริงมีตัวเลข เครื่องหมาย
 * และตัวย่อปนอยู่เสมอ (เช่น "Monthly OPD Statistics 2026") สิ่งที่ต้องกันคือค่าที่
 * ไม่มีอักษรอังกฤษอยู่เลย เช่นพิมพ์ชื่อไทยซ้ำลงช่องภาษาอังกฤษ
 *
 * นับแค่ a–z A–Z ไม่รวมอักษรโรมันที่มีเครื่องหมายเสริม (é, ü) — ช่องเหล่านี้ขอ
 * ภาษาอังกฤษ ไม่ใช่อักษรละตินทั้งบล็อก
 */
...
/**
 * เลขต่อของเบอร์โทรศัพท์ — ตัวเลขล้วน 1–10 หลัก
 *
 * เป็นช่องแยกจากเบอร์ ไม่ใช่ส่วนท้ายของเบอร์ (การ์ด "Field เบอร์โทร ให้เพิ่ม ต่อ-1232
 * ของเบอร์โทรศัพท์"): เบอร์ราชการเป็นเบอร์กลางที่ต้องกดต่อ แต่ `phoneSchema` รับได้แค่
 * 9–10 หลัก คนกรอกจึงต้องเลือกระหว่างทิ้งเลขต่อกับโดนฟอร์มปฏิเสธ
 *
 * รับแค่ตัวเลข — ไม่รับ "ต่อ" "ext." ขีด หรือช่องว่างข้างใน เพราะช่องนี้**คือ**ช่องต่อแล้ว
 * คำเหล่านั้นคือคนพิมพ์ทั้งประโยคลงช่องเดียว ซึ่งจะไปโผล่บนเอกสารเป็น "ต่อ ต่อ 1232"
 * สิบหลักเผื่อไว้กว้าง ๆ (ตู้สาขาราชการใช้ 3–5 หลัก) คอลัมน์กว้าง 16
 */
...
/**
 * ค่าว่างและ null ผ่าน — "ไม่มีเลขต่อ" คือคำตอบปกติของช่องนี้ และคืนเป็น null เสมอ
 * เพื่อให้คอลัมน์เก็บ NULL ไม่ใช่ "" ปนกัน (ฟอร์มส่ง "" มาเมื่อผู้ใช้ลบเลขต่อออก
 * และ toApiShape() คืน null เมื่อไม่เคยมี — ทั้งสองความหมายเดียวกัน)
 *
 * key ที่**ไม่ได้ส่งมา**ยังเป็น undefined — transform วิ่งบน optional ด้วย ถ้าแปลงเป็น
 * null เหมือนกันหมด PATCH ที่ส่งมาแค่ช่องเดียวจะลบเลขต่อของทุกคนทิ้งเงียบ ๆ
 */
export const phoneExtensionSchema = z
  .string()
  .trim()
  .refine((v) => v === "" || isValidPhoneExtension(v), PHONE_EXTENSION_MESSAGE)
  .nullable()
  .optional()
...
/**
 * ตำแหน่งในฟอร์มลงทะเบียนหน่วยงาน — ต้องมีภาษาไทย ปนภาษาอังกฤษและตัวเลขได้
 *
 * ใช้กับ **ทั้งสองคน**: ผู้มีอำนาจกระทำการแทน (`signatoryPosition`) และผู้ประสานงานของหน่วยงาน
 * (`contactPosition`) ป้ายกำกับของทั้งคู่เขียนว่า "ตำแหน่ง (ชื่อเต็มภาษาไทย)" กฎนี้คือการ
 * บังคับตามป้าย — ไม่ใช่ให้ป้ายขอไว้เฉย ๆ แล้วรับอะไรก็ได้
 *
 * เกณฑ์เดียวกับ `organizationNameSchema` — **มี**อักษรไทย ไม่ใช่ **เป็น**ภาษาไทยล้วน
 * เพราะตำแหน่งราชการจริงมีตัวย่ออังกฤษปนอยู่เสมอ (เช่น "ผู้อำนวยการศูนย์เทคโนโลยี
 * สารสนเทศและการสื่อสาร (ICT)") สิ่งที่ต้องกันคือค่าที่ไม่มีภาษาไทยอยู่เลยอย่าง
 * "Director General"
 *
 * ตำแหน่งของผู้มีอำนาจกระทำการแทนถูกพิมพ์ลงเอกสารข้อตกลง A0 ตรง ๆ ผ่าน
 * `org_approver.position` ซึ่งเป็นเอกสารราชการภาษาไทยที่มีการลงนาม ส่วนของผู้ประสานงานของหน่วยงาน
 * **template ชุดปัจจุบันยังไม่ได้พิมพ์ออกมา** (A0 อ้างถึงแค่ `org_officer.firstName`
 * กับ `.lastName`) — กฎที่นี่จึงมาจากข้อกำหนดของ BDI ว่าสองช่องนี้ต้องเหมือนกัน
 * ไม่ใช่จากสิ่งที่เอกสารพิมพ์อยู่วันนี้ แต่ `org_officer.position` เป็นตัวแปรที่
 * template เรียกใช้ได้อยู่แล้ว ฝ่ายกฎหมายเพิ่มลงเอกสารเมื่อไรก็ได้โดยไม่ต้องแก้โค้ด
 */
export const positionSchema = z
  ...
  .refine(
    containsThai,
    "ตำแหน่งต้องเป็นชื่อเต็มภาษาไทย (มีภาษาอังกฤษปนได้ เช่น ผู้อำนวยการศูนย์เทคโนโลยีสารสนเทศและการสื่อสาร (ICT))",
  );
