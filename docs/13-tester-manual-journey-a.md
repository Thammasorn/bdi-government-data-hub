...
| `firstnameTh` | ไม่บังคับ | ชื่อจริงตามเอกสารที่หน่วยงานส่งมา · ส่งมาก็ถูกนำไปเติมให้ในฟอร์มเปิดใช้งาน ไม่ส่งมาผู้ถูกเชิญกรอกเองตอนเปิดใช้งาน (ซึ่งยังบังคับกรอก) |
| `lastnameTh` | ไม่บังคับ | นามสกุลจริง · เงื่อนไขเดียวกับช่องบน |
...
| `409` | `role_occupied` | **หน่วยงานนี้มีคนถือบทบาทนี้ใช้งานอยู่แล้ว** หนึ่งหน่วยงานมีผู้ดำเนินการหนึ่งคนและผู้มีอำนาจอนุมัติหนึ่งคน ระบบไม่เปลี่ยนตัวให้ ข้อความบอกอีเมลคนที่ถืออยู่ — ต้องระงับ (`suspend`) หรือยุติบัญชี (`deactivate`) คนนั้นก่อน แล้วค่อยเชิญ |
| `409` | `invitation_pending` | หน่วยงานนี้มีคำเชิญบทบาทเดียวกันค้างอยู่ ที่นั่งถูกจองตั้งแต่ตอนเชิญ คำตอบคืน `activationKeyId` ของใบนั้น — จะเปลี่ยนคนให้ `DELETE` ใบเดิมก่อน (ข้อ 3) |
| `409` | `organization_not_active` | เชิญผู้มีอำนาจอนุมัติให้หน่วยงานที่ยังไม่เปิดใช้งาน — คนแรกต้องมาจากแบบฟอร์มจดทะเบียน (เล่ม B) เชิญตรงได้เฉพาะหน่วยงานที่ `ACTIVE` แล้วและที่นั่งว่าง |
...
`DELETE` จะลบบัญชีให้ก็ต่อเมื่อบัญชีนั้น ยังเป็น `PENDING` · ไม่มีคีย์ใบอื่นที่ยังใช้ได้ (ใบเก่าที่
`resend` ยกเลิกไปแล้วไม่นับ) · ยังไม่มีบทบาท ·
...
2. **คำนำหน้า ชื่อ นามสกุล** ระบบเติมมาให้แล้วและ **แก้ไขในหน้านี้ไม่ได้** — ชื่อกับ
   นามสกุลมาจากบัตรประชาชนที่คุณเพิ่งยืนยันผ่าน ThaID ส่วนคำนำหน้ามาจากที่เจ้าหน้าที่
   บันทึกไว้ตอนเชิญ (ThaID ไม่ได้ส่งคำนำหน้ามาให้) ถ้าช่องไหนยังว่างอยู่ ช่องนั้นกรอกได้
   หากชื่อที่ขึ้นไม่ตรงกับบัตร ให้ติดต่อผู้ประสานงานของ BDI ที่เชิญคุณ อย่าตั้งบัญชีทับไปก่อน
3. กรอก **เบอร์โทรศัพท์** — มือถือ 10 หลักขึ้นต้นด้วย `06` `08` หรือ `09` (เช่น `0812345678`)
   หรือเบอร์ที่ทำงาน 9 หลักขึ้นต้นด้วย `0` ตามด้วยรหัสพื้นที่ (เช่น `021234567`) ถ้าเป็นเบอร์กลาง
   ที่ต้องกดต่อ ใส่เลขต่อในช่อง **ต่อ** ข้าง ๆ (ตัวเลขล้วน เช่น `1232`) ไม่มีก็เว้นว่างไว้
4. ตั้ง **รหัสผ่าน** ให้ครบทั้งห้าข้อที่ขึ้นอยู่ใต้ช่อง — ยาวอย่างน้อย 12 ตัวอักษร และมี
   ตัวพิมพ์ใหญ่ `A-Z` · ตัวพิมพ์เล็ก `a-z` · ตัวเลข `0-9` · อักขระพิเศษ เช่น `! @ # $ ^ & * ( ) _ +`
   แต่ละข้อจะติ๊กถูกเองระหว่างพิมพ์
5. พิมพ์รหัสผ่านเดิมซ้ำในช่อง **ยืนยันรหัสผ่าน** — ไม่ตรงกันจะฟ้องทันที
6. กด **เปิดใช้งานบัญชี**

ภาพด้านบนคือตอนที่ใส่รหัสผ่านที่ยังไม่ครบข้อกำหนด จะเห็นว่าข้อความสีแดงขึ้นใต้ช่องที่มีปัญหา
...
- [ ] เชิญผู้ดำเนินการให้หน่วยงานที่มีผู้ดำเนินการใช้งานอยู่แล้ว → ได้ `409 role_occupied` บอกอีเมลคนเดิม
- [ ] `suspend` คนเดิม แล้วเชิญซ้ำ → ได้ `201` · เชิญคนที่สามทับ → ได้ `409 invitation_pending`
- [ ] `reinstate` คนเดิมก่อนที่คนใหม่จะกดลิงก์ → หน้าเปิดใช้งานขึ้นว่ายังเปิดไม่ได้ ลิงก์ยังใช้ได้
- [ ] เชิญผู้มีอำนาจอนุมัติให้หน่วยงานที่ยัง `PENDING_REGISTRATION` → ได้ `409 organization_not_active`
