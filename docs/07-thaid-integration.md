...
**และเปลี่ยนอีกครั้งเมื่อ 2026-09-07** เป็น
`openid pid given_name family_name given_name_en family_name_en title` ตามการ์ด
**ใช้ title จาก thaid** — `title` กลับเข้ามาเพราะระบบต้องการคำนำหน้าจากบัตร ไม่ใช่
คำนำหน้าที่ใครกรอกแทนกัน (ดูข้อ 4.1.1)

> ผลของค่าตั้งต้นชุดนี้: ฟอร์มสร้างบัญชีเติม **คำนำหน้า ชื่อ และนามสกุล** จากบัตรให้ครบ
> และล็อกทั้งสามช่อง ผู้ใช้เลือกคำนำหน้าเองเฉพาะเมื่อกรมการปกครองไม่ส่ง claim `title`
> กลับมา (`toIdentity()` ยังอ่าน `name` `name_en` อยู่ ถ้าวันหนึ่งกรมการปกครองส่งมาให้
> เองก็ได้ค่าเพิ่มมาโดยไม่ต้องแก้โค้ด)
>
...
### 4.1.1 `title` — ยิงจริงเมื่อ 2026-09-07

การ์ด **ใช้ title จาก thaid** ขอให้คำนำหน้ามาจากบัตร คำถามแรกคือ client ของโครงการ
ได้ scope `title` หรือยัง ไม่ใช่คำถามเรื่องโค้ด จึงยิง `/api/v2/oauth2/auth/` ดูก่อน
ด้วย client ของโครงการและ `redirect_uri` โดเมนจริงชุดเดียวกับที่ `main` ใช้

| scope ที่ขอ | ผล |
|---|---|
| `openid pid given_name family_name given_name_en family_name_en` (ค่าตั้งต้นเดิม) | 302 ไปหน้า QR ✅ |
| `openid pid title` | 302 ไปหน้า QR ✅ |
| `openid pid given_name family_name given_name_en family_name_en title` | 302 ไปหน้า QR ✅ ← **ค่าตั้งต้นใหม่** |
| `openid pid … title title_en` | 302 ไปหน้า QR ✅ |
| `openid nonsense_scope_zzz` | 400 `invalid_scope` ❌ |

แถวสุดท้ายมีไว้ด้วยเหตุผลเดียวกับตารางในข้อ 4.1 — ถ้าไม่ยิง แถวบน ๆ อ่านได้สองแบบ
`title` อยู่ใน `scopes_supported` ของ discovery document อยู่แล้ว (ข้อ 4.4) ผลนี้จึง
ตรงกับที่ประกาศไว้

ไม่ขอ `title_en` แม้จะผ่าน เพราะไม่มีที่เก็บใน `user_account` และไม่มีหน้าจอไหนใช้

**สิ่งที่ยังพิสูจน์ไม่ได้ด้วยการยิง**: authorize ผ่านแปลว่าทะเบียนอนุญาต scope นั้น
ไม่ได้แปลว่า id_token จะมี claim `title` กลับมาจริง ต้องมีคนถือบัตรจริงสแกนหนึ่งครั้ง
ถึงจะรู้ — เหมือนกับ `pid` เมื่อ 2026-09-02 ถ้าไม่มาก็ไม่พัง: บัญชีนั้นไม่มีคำนำหน้า
`lockedProfile()` จึงไม่ล็อกช่องนั้น และหน้า `/activate` แสดง dropdown ให้เลือกเองตามเดิม
