...
ตอน C   คุณลงทะเบียนชุดข้อมูล → ผ่านการตรวจ 4–5 ด่าน → ได้เอกสารฉบับอนุมัติ
```
...
1. **ที่อยู่เว็บของระบบทดสอบ** เช่น `http://localhost:3400` หรือ `https://bdi.thammasorn.org`
   (ภาพในคู่มือนี้ถ่ายจากเครื่องที่อยู่ที่ `http://localhost:3400`)
2. **อีเมลที่ได้รับคำเชิญ** ส่วนรหัสผ่านคุณจะตั้งเองตอนเปิดใช้งานบัญชี
3. **ช่องทางรับรหัส OTP** เพราะเครื่องทดสอบส่วนใหญ่ไม่ได้ส่งอีเมลจริง — อ่านข้อ 4 ประกอบ
...
> ทางนี้ใช้ได้เฉพาะบัญชีที่เคยเปิดใช้งานด้วย ThaiD ไปแล้ว
> ถ้าเครื่องทดสอบยังไม่ได้ตั้งค่าเชื่อมต่อกับ ThaiD กดแล้วจะขึ้นข้อความบอกว่ายังไม่ได้ตั้งค่า
...
ระบบมีอีเมลที่ส่งออกทั้งหมด 15 แบบ (คำเชิญ · รหัส OTP · แจ้งผลการตรวจสอบ ฯลฯ)
แต่ **เครื่องทดสอบส่วนใหญ่ตั้งค่าไว้ไม่ให้ส่งอีเมลจริง** เพื่อไม่ให้ไปรบกวนกล่องจดหมายของใคร
ถ้าคุณกดขอ OTP แล้วไม่มีอะไรเข้ามาในอีเมล มักไม่ใช่ระบบเสีย แต่เป็นเพราะเหตุนี้

| เครื่องทดสอบตั้งค่าไว้แบบไหน | คุณจะได้รหัส OTP และลิงก์คำเชิญจากที่ไหน |
| --- | --- |
| **ส่งอีเมลจริง** | เปิดกล่องจดหมายตามปกติ (เผื่อดูโฟลเดอร์ Spam กับ Promotions ด้วย) |
| **ไม่ส่งจริง** (ค่าปกติของเครื่องทดสอบ) | **ขอจากผู้ดูแลระบบ** เพราะระบบพิมพ์รหัสไว้ในบันทึกการทำงานของเครื่องแทน |

คำสั่งสำหรับผู้ดูแลระบบ (พิมพ์บนเครื่องที่ติดตั้งระบบ ในโฟลเดอร์ที่มีไฟล์ `docker-compose.yml`)
...
docker compose logs delivery-worker --tail 40 | grep -E 'ถึง:|เรื่อง:'   # อีเมลแจ้งเตือนที่ส่งออกไป
```
...
| ไฟล์แนบของหน่วยงาน | PDF หรือ JPG ขนาดไม่เกิน 10 MB | — |
...
| ไม่มีรหัส OTP เข้าอีเมล | เครื่องทดสอบไม่ได้ส่งอีเมลจริง | ขอรหัสจากผู้ดูแลระบบ (ข้อ 4) |
